-- ln: make symlinks (M2, ported from OpenOS).
-- Links are virtual (RAM-only, lost on reboot), like OpenOS.
-- OC has no inodes, so plain ln links symbolically too.
local fs = require("filesystem")
local shell = require("shell")

local args = shell.parse(...)
if #args == 0 then
  io.write("Usage: ln [-s] <target> [<name>]\n")
  return 1
end
