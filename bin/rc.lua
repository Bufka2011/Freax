-- rc: service control (OpenOS port). Freax boots via systemd, so this
-- is provided for /etc/rc.d compatibility. The OpenOS `rc` library is
-- inlined here since Freax ships no lib/rc.lua.
local fs = require("fs")
local rc = { loaded = {} }
...
local function loadConfig()
  ...
  if result then
    result, reason = xpcall(result, traceback)
    if result then
      return env
    end
  end
...
local function load(name, args)
  ...
  local env = setmetatable({args = args}, {__index = _ENV})
...
local function rawRunCommand(conf, name, cmd, args, ...)
  ...
  if result then
    ...
    elseif type(result[cmd]) == "function" then
      result, what = xpcall(result[cmd], traceback, ...)
      if result then
        return true
      end
    elseif cmd == "restart" and type(result["stop"]) == "function" and type(result["start"]) == "function" then
      local daemon = result
      result, what = xpcall(daemon["stop"], traceback, ...)
      if result then
        result, what = xpcall(daemon["start"], traceback, ...)
        if result then
          return true
        end
      end
