-- flash: read/write EEPROM (M2, ported from OpenOS).
-- Uses the mediated eeprom lib (first EEPROM found).
...
local function readRom()
  ...
  if not options.q then
    ...
    io.write("Reading EEPROM " .. tostring(eeprom.address()) .. ".\n")
  end
  ...
  file:close()
  if not options.q then
    io.write("All done!\nThe label is '" .. tostring(eeprom.getLabel()) .. "'.\n")
  end
  return nil
end
...
local function writeRom()
  ...
  if not options.q then
    io.write("Flashing EEPROM " .. tostring(eeprom.address()) .. ".\n")
  ...
  if label and #label > 0 then
    ...
    if not options.q then
      io.write("Set label to '" .. tostring(eeprom.getLabel()) .. "'.\n")
    end
  end
